...
export default function DomoticsPage() {
  ...
  const smartDevices = t("services.detail.domotica.smartDevices.devices", { returnObjects: true });
  const deviceIcons = [Lightbulb, Camera, DoorOpen, Droplets, Speaker, Waves];
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 relative overflow-hidden ${
      ...
      <div className="mx-auto max-w-6xl px-2 sm:px-4 py-6 sm:py-10 relative z-10">
        <ServiceHero
          title={t("services.detail.domotica.heroTitle")}
          image={service.image}
          icon={service.icon}
          variant="overlay"
          showBackButton={true}
            ...
            <div className="relative z-10 max-w-3xl mx-auto text-center">
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4 ${
                ...
                <span className={`text-sm font-semibold ${isDarkMode ? "text-brand-cyan-light" : "text-brand-cyan-dark"}`}>
                  {t("services.detail.domotica.cta.badge")}
                </span>
              </div>
              ...
              <h3 className={`text-3xl font-bold mb-3 ${
                ...
                {t("services.detail.domotica.cta.title")}
              </h3>
              <p className={`text-base mb-8 ${
                isDarkMode ? "text-dark-text-secondary" : "text-light-text-secondary"
              }`}>
                {t("services.detail.domotica.cta.description")}
              </p>
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className={`text-2xl font-bold ${
              ...
              {t("services.detail.domotica.smartDevices.title")}
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {devicesWithIcons.map((device, index) => (
              <SmartDeviceCard
                key={index}
                title={device.title}
        ...
        <ServiceFeatures
          features={t("services.detail.domotica.features", { returnObjects: true })}
          variant="cards"
          title={t("services.detail.domotica.featuresTitle")}
        />
      </div>
    </div>
  );
